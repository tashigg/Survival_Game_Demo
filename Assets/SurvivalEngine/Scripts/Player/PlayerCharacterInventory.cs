    ...
    public class PlayerCharacterInventory : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            foreach (KeyValuePair<int, InventoryItemData> item in EquipData.items)
            {
                if (item.Value != null)
                {
                    if (equipped_data.Contains(item.Value.item_id))
                    {
                    ...
                    if (!equipped_items.ContainsKey(item.Value.item_id))
                    {
                        ...
                        {
                            Debug.LogError("Owner equip");
                            EquipAddedItem(item.Value.item_id);
                            equipmentNetwork.OnEquipItem(character, item.Value.item_id);
                        }
                    }
                }
            }
        ...
        public void ClientTakeItem(Item item)
        {
            if (BagData != null && !InventoryData.CanTakeItem(item.data.id, item.quantity) && !item.data.IsBag())
            {
                TakeItem(BagData, item, false); //Take into bag
            }
            else
            {
                TakeItem(InventoryData, item, false); //Take into main inventory
            }
        }

        public void TakeItem(InventoryData inventory, Item item, bool IsLocalPlayer = true)
        {
        ...
        public EquipmentNetwork equipmentNetwork;
        ...
        public void EquipAddedItem(string item_id)
        {
        ...
        public void ClientEquipItem(string item_id)
        {
            ItemData idata = ItemData.Get(item_id);
            GameObject equip_obj = Instantiate(idata.equipped_prefab, transform.position, Quaternion.identity);
            EquipItem eitem = equip_obj.GetComponent<EquipItem>();
